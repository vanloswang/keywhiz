...
public class AutomationSecretGeneratorsResourceTest {
  ...
  AutomationSecretGeneratorsResource resource;

  @Before public void setUp() {
    resource = new AutomationSecretGeneratorsResource(objectMapper,
  ...
  @Test public void generates() throws Exception {
    when(generator.generate(eq(automationClient.getName()), eq(params)))
        .thenReturn(ImmutableList.of(secret));
    List<SanitizedSecret> sanitizedSecrets =
        resource.generate(automationClient, generatorName, objectMapper.writeValueAsString(params));
